...
def scrape_with_playwright():
    ...
    with sync_playwright() as pw:
        ...
        # Use the search URL directly with per_page=50 and empty filters
        page_num = 1
        while True:
            search_url = (
                f'https://www.njlispendens.com/member/property'
                f'?per_page=50&page={page_num}&search=Search'
            )
            page.goto(search_url, wait_until='domcontentloaded')
            page.wait_for_timeout(3000)

            # Re-check login in case session expired
