...
def load_seen():
    if TEST_MODE:
        print('TEST MODE: clearing seen_properties cache for fresh 14-day run')
        return set()
    try:
...
def scrape():
    ...
    r = s.get('https://www.njlispendens.com/member/property',
              params={'County[0]':'0','date_added':str(LOOKBACK_DAYS),'per_page':'50','search':'Search','cp':'0'})
    ...
    for row in soup.select('table tbody tr'):
        ...
        except Exception as e: print('parse error: ' + str(e))
    return props
...
def main():
    mode_str = 'TEST MODE ('+str(LOOKBACK_DAYS)+'-day lookback, no cache)' if TEST_MODE else str(LOOKBACK_DAYS)+'-day lookback'
    print('NJ Foreclosure Bot - ' + datetime.now().strftime('%Y-%m-%d %H:%M') + ' | ' + mode_str)
    seen = load_seen()
    props = scrape()
    new_props = [p for p in props if p['docket'] not in seen]
    print('Scraped '+str(len(props))+', '+str(len(new_props))+' to process')
